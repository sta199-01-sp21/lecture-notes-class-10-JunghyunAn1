...
The probability of matching colors would be less than the probility of 
different colors.
...
We can use mutate to funciton to return a boolean  value to see if 'draw_1' and
'draw_2' match.
...
```{r simulate_rolls}
roll_results <- tibble(
  die_1 = replicate(n = 10000, sample(1:6, size =1)),
  die_2 = replicate(n = 10000, sample(1:6, size =1))
)
```
